    ...
    try {
        ...
        if (req.file) {
            ...
            try {
                ...
                if (!resumeText) {
                    console.warn("⚠️ Resume file provided but no text could be extracted.");
                } else {
                    console.log("✅ Successfully extracted text from PDF resume.");
                }
            } catch (err) {
                console.error("❌ Failed to parse PDF resume:", err.message);
                // We continue with empty resumeText if selfDescription is available
            } finally {
        ...
        if (!req.file && !req.body.selfDescription) {
            return res.status(400).json({
                message: "Provide a resume or self description"
            });
        }
